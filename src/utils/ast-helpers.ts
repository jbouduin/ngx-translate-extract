...
export function findEnumDeclaration(node: SourceFile, enumIdentifier: Node): Array<string> | null {
	let result: EnumDeclaration = null;
	if (isIdentifier(enumIdentifier) && enumIdentifier.escapedText) {
		const queryWhereEnum = `ImportDeclaration:has(ImportSpecifier[text=${enumIdentifier.escapedText}]) StringLiteral`;
		const [where] = tsquery<StringLiteral>(node, queryWhereEnum);
		const query = `EnumDeclaration:has(Identifier[name=${enumIdentifier.escapedText}])`;
		if (!where) {
			[result] = tsquery<EnumDeclaration>(node, query);
		} else {
			const __dirname = Path.parse(node.fileName).dir;
			...
			for (const path of filePath) {
				...
				if (enumFile) {
					...
					[result] = tsquery<EnumDeclaration>(contents, query);
				}
			}
		}
		if (result != null) {
			const returnValue = new Array<string>();
			for (const enumMember of result.members) {
				if ((<Identifier>enumMember.name).escapedText) {
					returnValue.push(<string>(<Identifier>enumMember.name).escapedText);
				}
			}
			return returnValue;
		} else {
