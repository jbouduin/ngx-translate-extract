...
describe('MarkerParser', () => {
	...
	it('should extract strings using marker function', () => {
		const contents = `
			import { marker } from '@jbouduin/ngx-translate-extract';
			marker('Hello world');
	...
	it('should extract split strings', () => {
		const contents = `
			import { marker} from '@jbouduin/ngx-translate-extract';
			marker('Hello ' + 'world');
			marker('This is a ' + 'very ' + 'very ' + 'very ' + 'very ' + 'long line.');
			marker('Mix ' + \`of \` + 'different ' + \`types\`);
		`;
		const keys = parser.extract(contents, componentFilename).keys();
	...
	it('should extract split strings while keeping html tags', () => {
		const contents = `
			import { marker } from '@jbouduin/ngx-translate-extract';
			marker('Hello ' + 'world');
			marker('This <em>is</em> a ' + 'very ' + 'very ' + 'very ' + 'very ' + 'long line.');
			marker('Mix ' + \`of \` + 'different ' + \`types\`);
		`;
		const keys = parser.extract(contents, componentFilename).keys();
